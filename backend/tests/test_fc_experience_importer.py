...
def _build_workbook(rows: list[tuple], *, headers: tuple[str, ...] = TEMPLATE_HEADERS) -> bytes:
    workbook = Workbook()
...
def test_parse_experience_case_excel_success() -> None:
    ...
    assert result.valid_rows[0].tags is None
    assert result.valid_rows[1].case_type == "negative"
    assert result.valid_rows[1].priority == "P1"


def test_parse_experience_case_excel_with_tags() -> None:
    content = _build_workbook(
        [
            (
                "EXP-001",
    ...
    assert len(result.valid_rows) == 2
    assert result.valid_rows[0].tags == "登录,冒烟"
    assert result.valid_rows[1].tags is None
